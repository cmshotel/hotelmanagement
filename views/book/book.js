...
function showquatation() {
	...
	if ($('#quotetab').length == 0) {
		...
		totromcost = 0;
		totextrabedcost = 0, totchildcost = 0, totplancost = 0, totcosttohot = 0, finalcost = 0, totstaygst = 0, totfoodgst = 0;
		...
		$('#quotetab').append('<tr><td>Child without extrabed(s) Charges (₹) :</td><td><input type="number" min="0" name="childwoexcost" value="' + totchildwoextracost + '"</td></tr>');
...
function dobooking() {
	...
	$.ajax({
		...
		data: JSON.stringify({
			...
			"plan": plan,
			"exbed": extrabed,
			"cwextrabed": cwextrabed,
			"cwoextrabed": cwoextrabed,
			"roomprice": roomcost,
			"totroomcost": totroomcost,
			"plancharges": plancost,
			"extrabed": extraperson,
			"childwexcost": childwex,
			"childwoexcost": childwoex,
			"tothotelcost": tothotelcost,
			"staygstrate": staygstrate,
