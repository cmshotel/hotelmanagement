  ...
  console.log("/avibility");
	...
	app.post('/getcat', function(req, res) {
	    mongo.connect(murl, function(err, db) {
	        ...
	        var cursor = db.collection('common').find({}).project({ catagories:1,plans:1, _id: 0 });
	        cat = [], pln = [];
...
//insert availibility
	...
	app.post('/geteditdata', function(req, res) {
	    ...
	    mongo.connect(murl, function(err, db) {
	        assert.equal(null, err);
	        var collection = db.collection('avail');
	        collection.find({"date":edate }).toArray(function(err, docs) {
	            assert.equal(err, null);
	            var jso=docs[0];
	            res.send(jso);
	        });
	...
	app.post('/updateavail', function(req, res) {
	  ...
	  jso={"date":edate,"room":Rooms,"Extrabed":extrabed,"coupon":prom,"Plans":plans}
	    mongo.connect(murl, function(err, db) {
	        assert.equal(null, err);
	        var collection = db.collection('avail');
					collection.update({"date":edate}, {$set:jso}, function(err, result) {
						var resp;
		    if (err){
					resp={"res":"failure"}
				}
...
app.get('/booking', function (req, res) {
  ...
  console.log("/booking/index.ejs");
	...
	app.post('/getavail', function(req, res) {
		...
		mongo.connect(murl, function(err, db) {
			...
			cursor.forEach(function(doc, err) {
			    ...
			    jso = {
			    	"jsa": resarr,
			    	"plans":{"plannames":finplans,"planprices":finplanprices},
	...
	app.post('/getcatagory', function(req, res) {
	    ...
	    mongo.connect(murl, function(err, db) {
	        assert.equal(null,err);
	        db.collection('avail').find({"date": {$gte: chkin,  $lt: chkout }}).toArray(function(err,docs){
	           assert.equal(null, err);
	            ...
	            db.close()
	            res.send({"catlist":catlist,"planlist":planlist});
	        });
	    });
	});      
	...
	app.post('/contectsuggest', function(req, res) {
	    ...
	    mongo.connect(murl, function(err, db) {
	        assert.equal(null,err);
	        db.collection('booking').find({"contact":new RegExp(cont),"chkin":chkin,"chkout":chkout,"catagory":cat}).toArray(function(err,docs){
	            assert.equal(null, err);
	            for(i=0;i<docs.length;i++){            
	...
	app.post('/getinfo', function(req, res) {
	    ...
	    mongo.connect(murl, function(err, db) {
	        assert.equal(null,err);
	        db.collection('booking').find({"contact":cont,"chkin":chkin,"chkout":chkout,"catagory":cat}).toArray(function(err,docs){
	            assert.equal(null, err);       
	            db.close()
	            res.send(docs[0]);
	        });
  ...
  console.log("/history");
	...
	app.post('/getview', function(req, res) {
	    ...
	    if(flg.localeCompare("bookdate")==0){
	        mongo.connect(murl, function(err, db) {
	            assert.equal(null,err);
	            db.collection('booking').find({"bookdate":{$gte: from,$lte:to}}).toArray(function(err,docs){
	                assert.equal(null, err);       
	                db.close()
	                res.send(docs);
	            });
	    ...
	    if(flg.localeCompare("chkindate")==0){
	        mongo.connect(murl, function(err, db) {
	            assert.equal(null,err);
	            db.collection('booking').find({"chkin":{$gte: from,$lte:to}}).toArray(function(err,docs){
	                assert.equal(null, err);       
	                db.close()
	                res.send(docs);
	            });
