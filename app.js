...
for(i=0;i<stayslabs;i++){
	...
	if(req.body[b].toString().localeCompare("")==0 || req.body[b]==null){
		temp={"range":req.body[a],"rate":req.body[c]};
	}
	else{
	temp={"range":req.body[a]+"-"+req.body[b],"rate":req.body[c]};}
...
docs=[{"catagories":categ,"plans":plan},{"gst":[{"stayslabs":stayslabarr},{"foodslabs":foodslabarr}]}];
